#' Function that computes sider enrichment with outcome
#'
...
#' @param sider The sider database, read into memory
#' @param useBayes Use bayesian regression from arm package (1=yes, 0=no (default) )
#' @examples
#' querySider(dataset=admit.drug.formula.subset,
...
querySider <- function(dataset=admit.drug.formula.subset,
                       drugcolumnname=drugcolumnname,
                       patientcolumnname=patientcolumnname,
                       outcomecolname=outcomecolname,
                       covariatecols=covariatecols,
...
cat("Querying Sider for Enrichments...\n");

  setnames(sider, c("drug_name", "side_effect_name","UMLS_concept_id"), c("drug","event",'umls_id') )

  # Change med_name to lower case to match lower-case names in sider table
  admit.drug.exposure[, med_name_lc:=tolower( get(eval(drugcolumnname ))) ]

  # Set keys for admit.drug and sider before merging
  ...
  setkey(sider, drug, event) # we'll match med_name to drug

  #################################################################
  ### Get intersecting names in each datasets
  #admit.drug.exposure.drugnames <- (unique(admit.drug.exposure$med_name_lc)); sider.drugnames <- unique(sider$drug);
  #admit.drug.drugnamesINTERSECToffides.drugnames <- intersect( admit.drug.exposure.drugnames, sider.drugnames )

  ### Print a table of drug counts in each dataset, along with intersection
  # kable(data.frame('Dataset Drug Count'=c(length(admit.drug.drugnames), length(sider.drugnames), length(admit.drug.drugnamesINTERSECToffides.drugnames)),row.names = c("HF Admissions","Side Effects","Intersection")))

  # Clean up variables only used for tables
  #rm(admit.drug.exposure.drugnames, admit.drug.drugnamesINTERSECToffides.drugnames, sider.drugnames);
  #################################################################

  #################################################################
  ###### Create subsets of the two datasets to merge together in the next step ######
  admit.drug.subset <- admit.drug.exposure # extract relevant columns of admit.drug
  sider.subset <- sider[,.(drug, umls_id, event)] # extract relevant columns of 1-way side effects

  ############ Merge of relevant columns of admit.drug and sider ############
  admit.drug.se <- merge(admit.drug.subset, sider.subset, by.x='med_name_lc', by.y='drug', allow.cartesian=TRUE) #
  ...
  rm(admit.drug.subset, sider.subset);
...
#   }
  ...
  estimate_df4 <- as.data.frame(matrix(ncol=4, nrow=length(nDrug_columns)), data=NA)
  colnames(estimate_df4) <- c("Estimate", "Std..Error", "z.value", "Pr...z..")
  ...
  for(i in seq_along(nDrug_columns)){
    ...
    estimate_df4[i,] <- data.frame(coef(summary(out)))[2,]
    setTxtProgressBar(pb, i)
  }
  close(pb)

  ### Make row names variable names
  row.names(estimate_df4) <- nDrug_columns
  colnames(estimate_df4) <- c('Estimate', 'Std.Error', "Z_value","P_value")

  ### Order columns by P-value
  estimate_df4 <- estimate_df4[with(estimate_df4, order(P_value)), ]

  ### FDR significant P-values
  FDR.pval <- p.adjust(estimate_df4$P_value, method="fdr")
  estimate_df4 <- cbind(estimate_df4, FDR.pval)

  ### Rework dataset into data.table formate
  estimate_df4 <- data.table(cbind(rownames(estimate_df4), estimate_df4))
  setnames(estimate_df4, "rownames(estimate_df4)", "Effect.Exposure")

  ### Create indicator variable for FDR Significance
  estimate_df4[,FDR.Sig:=ifelse(FDR.pval<0.05,1,0), by=.(Effect.Exposure)]

  ### Make the drug name column look better by removing "nDrugs_" before the drug name
  estimate_df4$Effect.Exposure <- gsub("^.*?_", "", as.character(estimate_df4$Effect.Exposure))

  ### Add OR to table (exponentiated coefficients)
  estimate_df4[, OR:=exp(Estimate), by=.(Effect.Exposure)]

  return(estimate_df4)

  ### Table of primary side effect exposure odds ratios
  #setkey(estimate_df4, P_value) ;
  #estimate_df4[order(P_value)]  ;
  #kable(estimate_df4[P_value<0.05]) ;
